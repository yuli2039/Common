 ...
 */
package com.yu.picmultiselect.listadapter;
...
public abstract class BaseQuickAdapter<T, H extends BaseAdapterHelper> extends BaseAdapter {
	...
	/**
	 * 多条目类型的listview,使用此构造方法，需要传入一个MultiItemTypeSupport参数
	 */
		...
		if (displayIndeterminateProgress) {
			if (mMultiItemSupport != null)
				return position >= data.size() ? 0
						: mMultiItemSupport.getItemViewType(position, data.get(position));
		} else {
			if (mMultiItemSupport != null)
